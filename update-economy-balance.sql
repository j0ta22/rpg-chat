-- Update economy balance: reduce starting gold and adjust item prices

-- 1. Update starting gold from 100 to 50
ALTER TABLE users ALTER COLUMN gold SET DEFAULT 50;

-- 2. Update existing users who have exactly 100 gold (new players) to 50
UPDATE users SET gold = 50 WHERE gold = 100;

-- 3. Update item prices for better economy balance

-- Disable RLS temporarily for price updates
ALTER TABLE items DISABLE ROW LEVEL SECURITY;

-- Clear existing items
DELETE FROM items;

-- Insert items with balanced prices
