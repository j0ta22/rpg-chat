-- Fix Row Level Security (RLS) for all public tables - FINAL VERSION
-- This script enables RLS and creates appropriate policies for game tables

-- 1. First, fix the player_inventory table structure (add missing id column)
...
DROP POLICY IF EXISTS "Users can view their own combats" ON public.combats;
DROP POLICY IF EXISTS "Users can insert their own combats" ON public.combats;
DROP POLICY IF EXISTS "Users can update their own combats" ON public.combats;
...
-- 4. Create policies for combats table (using correct column names)
...
-- 5. Create policies for items table
...
-- 6. Create policies for player_inventory table
...
-- 7. Create policies for weekly_champions table
...
-- 8. Create policies for user_equipment table
...
-- 9. Create policies for users table (if not already exists)
