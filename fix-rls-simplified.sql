-- Simplified RLS fix that only works with existing tables
-- This script will check what tables exist and only fix RLS for those tables
...
-- Saved players - users can manage their own saved players (if table exists)
DO $$
BEGIN
