...
console.log('🔄 Updating combat system with rewards and penalties...')

// 1. Update the WebSocket server
console.log('📝 Updating WebSocket server with rewards system...')
const serverPath = 'server/websocket-server.js'
const rewardsServerPath = 'server/websocket-server-with-rewards.js'

if (fs.existsSync(rewardsServerPath)) {
  // Backup original
  fs.copyFileSync(serverPath, serverPath + '.backup-rewards')
  console.log('💾 Backed up original server to websocket-server.js.backup-rewards')
  
  // Replace with rewards version
  fs.copyFileSync(rewardsServerPath, serverPath)
  console.log('✅ Updated WebSocket server with rewards system')
} else {
  console.log('❌ Rewards server file not found')
}

// 2. Create a comprehensive migration guide
const migrationGuide = `# Combat Rewards System Implementation Guide
...
fs.writeFileSync('COMBAT_REWARDS_IMPLEMENTATION_GUIDE.md', migrationGuide)
console.log('📚 Created implementation guide: COMBAT_REWARDS_IMPLEMENTATION_GUIDE.md')

console.log('🎉 Combat rewards system update complete!')
console.log('')
console.log('Next steps:')
console.log('1. Execute combat-rewards-migration.sql in Supabase')
console.log('2. Test the enhanced combat system')
console.log('3. Verify rewards are calculated correctly')
console.log('4. Check level difference penalties work')
console.log('5. Deploy the updated server')
